...
public class CertTest {
    ...
    private static void certVerify() throws IOException, InvalidSignDataException, InvalidKeyDataException {
...
//        byte[] certContent = new byte[certBytes.length - 71];//证书内容
//        byte[] certSign = certInfo.getSignature().getBytes();//证书的签名71bytes
//        byte[] parentPublicKey = publicInfo.getPublicKeyData().getBytes();//由于是根证书, 因此主体公钥即为根证书公钥, 如果非根证书, 则需要父级证书的公钥
//
//        System.arraycopy(certBytes, 0, certContent, 0, certContent.length);
//
//        SM2Cipher cipher = new SM2Cipher(SM2Cipher.Type.C1C3C2);
//        System.out.println(cipher.verifySignByASN1(null, parentPublicKey, certContent, certSign));//证书内的签名字段为ASN.1编码
    }
